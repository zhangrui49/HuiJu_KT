 ...
 */
class MovieTabPresenter(context: Context, view: MovieTabContract.View) : MovieTabContract.Presenter(context, view) {

    override fun requestData(type: String, map: HashMap<String, Any>) {
        mView?.showLoading()
        addSubscription(RetrofitClient.getDoubanInstance(mContext!!).create(Api.DoubanApi::class.java)!!.getMovies(type, map), object : ApiCallBack<Movie>() {
